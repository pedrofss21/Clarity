...
// Campo de pesquisa (certifique-se de que exista no HTML)
...
function updateSummary() {
    ...
    const total = cars.reduce((soma, carro) => {

        return soma + carro.valor;
...
function pesquisarVeiculos(texto) {
    ...
    const resultado = cars.filter(carro => {
        ...
        const data = new Date(carro.data + "T00:00:00")
            .toLocaleDateString("pt-BR");
...
function updateSummary(lista = cars) {
    ...
    const total = lista.reduce((soma, carro) => {
...
function gerarPDF() {
    ...
    lista.forEach(carro => {
        ...
        tabela.push([
            ...
            new Date(carro.data).toLocaleDateString("pt-BR"),
